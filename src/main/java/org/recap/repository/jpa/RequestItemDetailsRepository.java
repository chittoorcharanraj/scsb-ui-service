...
public interface RequestItemDetailsRepository extends BaseRepository<RequestItemEntity> {
     ...
     * @return the page
     */
    @Query(value = "select request from RequestItemEntity request inner join request.itemEntity item inner join request.requestStatusEntity status where ((request.patronId = :patronBarcode OR :patronBarcode = '') and (request.requestingInstitutionId = :institutionId  OR :institutionId = 0) and (item.barcode = :itemBarcode OR :itemBarcode = '') and (status.requestStatusCode in ('RETRIEVAL_ORDER_PLACED', 'RECALL_ORDER_PLACED', 'EDD_ORDER_PLACED')) and (item.imsLocationId = :imsLocationId OR :imsLocationId = 0)) OR ((request.patronId = :patronBarcode OR :patronBarcode = '') and (item.barcode = :itemBarcode OR :itemBarcode = '') and (request.requestingInstitutionId not in (:institutionId)) and (item.owningInstitutionId = :institutionId OR :institutionId = 0) and (status.requestStatusCode in ('RETRIEVAL_ORDER_PLACED', 'RECALL_ORDER_PLACED', 'EDD_ORDER_PLACED')) and (item.imsLocationId = :imsLocationId OR :imsLocationId = 0))")
     ...
     */
    @Query(value = "SELECT REQUEST_TYPE_T.REQUEST_TYPE_CODE, REQUEST_ITEM_T.REQUESTING_INST_ID, ITEM_T.OWNING_INST_ID, ITEM_T.COLLECTION_GROUP_ID, ITEM_T.BARCODE, REQUEST_ITEM_T.CREATED_DATE,REQUEST_ITEM_STATUS_T.REQUEST_STATUS_CODE,ITEM_T.IMS_LOCATION_ID,ITEM_T.CALL_NUMBER, ITEM_T.CUSTOMER_CODE, REQUEST_ITEM_T.STOP_CODE " +
            " FROM  REQUEST_ITEM_T" +
            "    INNER JOIN ITEM_T ON REQUEST_ITEM_T.ITEM_ID = ITEM_T.ITEM_ID " +
